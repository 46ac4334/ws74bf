 ...
 *
 */
public class Plotter6166i extends JInternalFrame {
	...
	public class PlotterPane extends JPanel
		 ...
		 */
		public PlotterPane() {
			this.setPreferredSize(Plotter6166i.this.getPlotterPaneDefaultDimension());
			this.setBackground(Color.WHITE);
			this.insets = Plotter6166i.this.insets;
...
//			this.add(this.captionArea);
		...
		private void drawXTickLabels(final Graphics2D g, final PathIterator pathIterator2, final int sigDig) {
			...
			while (!pathIterator2.isDone()) {
				...
				final var tickText = Plotter6166i.this.xDateOrigin == Long.MIN_VALUE
						? String.format(format, coords[0]).replaceFirst("(\\.\\d*)0+($|e)", "$1$2")
						: Plotter6166i.getDateinstance().format(new Date(
								Plotter6166i.this.xDateOrigin + (long) coords[0] * Plotter6166i.MILLIS_PER_DAY));
		...
		private void drawYTickLabels(final Graphics2D g, final PathIterator pathIteratorY, final int sigDig) {
			while (!pathIteratorY.isDone()) {
				...
				final var format = "%." + String.valueOf(sigDig) + "g";
				final var yLabelValue = Plotter6166i.this.semiLog ? Math.pow(1e1, coords[1]) : coords[1];
...
//		}
		...
		private AffineTransform getPlotTransform(final double sx2, final double sy2, final AffineTransform pan2) {
			...
			if (Plotter6166i.this.pathToUnitBoxTransform == null) {
				throw new RuntimeException("pathToUnitBoxTransform is null");
			} else {
				plotTransform.concatenate(Plotter6166i.this.pathToUnitBoxTransform);
			}
			...
			if (this.markers != null) {
				...
				for (final Shape marker : this.markers) {
					if (marker != null && point != null && marker.contains(point)) {
						...
						final var tipTextAvailable = Plotter6166i.this.tips != null
								&& Plotter6166i.this.tips.size() > index && Plotter6166i.this.tips.get(index) != null;
						this.setToolTipText(
								tipTextAvailable ? Plotter6166i.this.tips.get(index) : String.format("%,d", index));
		...
		public void paint(final Graphics g0) {
			...
			this.insets = new Insets(Plotter6166i.this.top, Plotter6166i.this.left, Plotter6166i.this.bottom,
					Plotter6166i.this.right);
		...
		private void painter(final Graphics2D g, final int width, final int height) {
			try {
				...
				if (Plotter6166i.this.isShowCaption()) {
					this.add(this.captionArea);
				}
				...
				if (this.path != null || true) {
					...
					final var transformedXGrid = this.plotTransform.createTransformedShape(Plotter6166i.this.xGrid);
					final var transformedXGrid2 = this.plotTransform.createTransformedShape(Plotter6166i.this.xGrid2);
					final var transformedXGrid3 = this.plotTransform.createTransformedShape(Plotter6166i.this.xGrid3);
					final var transformedYGrid = this.plotTransform.createTransformedShape(Plotter6166i.this.yGrid);
					final var transformedYGrid2 = this.plotTransform.createTransformedShape(Plotter6166i.this.yGrid2);
					final var transformedYGrid3 = this.plotTransform.createTransformedShape(Plotter6166i.this.yGrid3);

					final var oldColor = g.getColor();
					g.setColor(Color.BLACK);

					final var xBounds = Plotter6166i.this.xGrid.getBounds2D();
					...
					this.drawXTickLabels(g, Plotter6166i.this.xGrid2.getPathIterator(null), sigDigX);

					if (Plotter6166i.this.xTicks2 != null) {
						if (Plotter6166i.this.xTicks2.length < 2) {
							this.drawXTickLabels(g, Plotter6166i.this.xGrid3.getPathIterator(null), sigDigX);
						}
						if (Plotter6166i.this.xTicks2.length + Plotter6166i.this.xTicks3.length < 2) {
							this.drawXTickLabels(g, Plotter6166i.this.xGrid.getPathIterator(null), sigDigX);
						}
					}

					final var yBounds = Plotter6166i.this.yGrid.getBounds2D();
					...
					final var sigDigY = (int) (2.4 + Math.log10(maxAbsY / yRange));// significant digits for Y tick
																					// labels
					this.drawYTickLabels(g, Plotter6166i.this.yGrid2.getPathIterator(null), sigDigY);
					if (Plotter6166i.this.yTicks2 != null) {
						if (Plotter6166i.this.yTicks2.length < 2) {
							this.drawYTickLabels(g, Plotter6166i.this.yGrid3.getPathIterator(null), sigDigY);
						}
						if (Plotter6166i.this.yTicks2.length + Plotter6166i.this.yTicks3.length < 2) {
							this.drawYTickLabels(g, Plotter6166i.this.yGrid.getPathIterator(null), sigDigY);
						}
					}
				...
				g.setClip(null);
				if (Plotter6166i.this.textItems != null) {
					final var iterator = Plotter6166i.this.textItems2.iterator();
					while (iterator.hasNext()) {
						final var next = iterator.next();
						next.plotText(g, this.plotTransform);
					}
				}
				if (Plotter6166i.this.plotTitle != null && !Plotter6166i.this.plotTitle.isEmpty()) {
					this.plotTheTitle(g, w);
				}

				for (final LegendItem li : Plotter6166i.this.legendItems) {
...
//					area.setOpaque(true);
		...
		private void plotTheTitle(final Graphics2D g, final int w) {
			...
			var font = Plotter6166i.this.titleFont;
			var count = 0;
			do {
				++count;
				if (!fitDone) {
				...
				stringBounds = fontMetrics.getStringBounds(Plotter6166i.this.plotTitle, g);
				x = w / 2 - stringBounds.getCenterX() + Plotter6166i.this.left;
				fitDone = count > 100
						|| x > 0 && x + stringBounds.getWidth() < Plotter6166i.this.left + w + Plotter6166i.this.right;
			} while (!fitDone);
			final var y = Plotter6166i.this.top / 2 - stringBounds.getCenterY();
			final var oldFont = g.getFont();
			final var oldColor = g.getColor();
			g.setFont(font);
			g.setColor(Plotter6166i.this.titleColor);
			g.drawString(Plotter6166i.this.plotTitle, (float) x, (float) y);
			g.setColor(oldColor);
			g.setFont(oldFont);
		}
...
//				this.s = 1;
				 ...
				 */
				Plotter6166i.this.createGrid(this.dataBounds, true);
...
//				final AffineTransform plotTransform2 = this.getPlotTransform(this.s, this.pan);
				...
				if (Plotter6166i.roundrangeDebug) {
	...
	public static DateFormat getDateinstance() {
		return Plotter6166i.dateInstance;
	}
	...
	public static boolean isRoundrangeDebug() {
		return Plotter6166i.roundrangeDebug;
	}
	...
	public static void setRoundrangeDebug(final boolean roundrangeDebug) {
		Plotter6166i.roundrangeDebug = roundrangeDebug;
	}
...
//	private Dimension plotterPaneDefaultDimension = new Dimension(865, 600);
	private Dimension plotterPaneDefaultDimension = new Dimension(460, 368);
	 ...
	 * @param arg0
	 */
	public Plotter6166i(final String arg0) {
		super(arg0);
		this.init();
	}
	...
	public void addPlotPath(final Stroke stroke, final Shape path, final Color color, final Color fillColor) {
		if (this.plotterPane.path == null) {
			...
			newButton.addActionListener(e -> Plotter6166i.this.plotterPane.repaint());
			newButton.setUI(Plotter6166i.newUI);
	...
	protected void copyImageToFile() throws IOException {
		...
		final var outFileName = Plotter6166i.getTimeStamp() + Plotter6166i.this.getTitle() + ".png";
	...
	private void createGrid(final Rectangle2D gridBounds, final boolean zooming) {
		if (Plotter6166i.roundrangeDebug) {// if debugging, print message with unique id "81930de4"
		...
		try {
			 ...
			 */
			final var xRoundRange = Plotter6166i.getRoundRange(gridBounds.getX(),
					gridBounds.getX() + gridBounds.getWidth());
			final var yRoundRange = Plotter6166i.getRoundRange(gridBounds.getY(),
...
//			}
	...
	private void init() {
		...
		menuItemSave.addActionListener(arg0 -> {
			try {
				Plotter6166i.this.copyImageToFile();
		...
		menuItemManagePlots.addActionListener(e -> {
			...
			if (this.controlPanel.getDesktopPane() == null) {
				 ...
				 */
				final var desktopPane2 = Plotter6166i.this.getDesktopPane();
...
//					this.controlPanel.getContentPane().setLayout(new FlowLayout());
		...
		menuItemQuit.addActionListener(arg0 -> Plotter6166i.this.shutdown());
